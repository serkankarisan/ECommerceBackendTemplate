...
namespace Business.Concrete.Shoppings
{
    public class OrderItemManager : IOrderItemService
    {
        ...
        {
            OrderItem? result = await _orderItemDal.GetAsync(p => p.Id == id);
        ...
        {
            Task<IDataResult<OrderItem>> updatedOrderItem = GetByIdAsync(orderItem.Id);
            if (updatedOrderItem == null)
            {
                return new ErrorResult(Messages.NotFound);
            }
            OrderItem result = await _orderItemDal.UpdateAsync(orderItem);
        ...
        public async Task<IResult> AddAsync(AddOrderItemDto addOrderItemDto)
        {
            ...
            OrderItem result = await _orderItemDal.AddAsync(orderItem);
            return result != null ? new SuccessResult(Messages.Added) : new ErrorResult(Messages.NotAdded);
        }
        public async Task<IResult> DeleteAsync(int id)
        {
            IDataResult<OrderItem> deletedOrderItem = await GetByIdAsync(id);
            if (deletedOrderItem == null)
            {
                return new ErrorResult(Messages.NotFound);
            }
            OrderItem result = await _orderItemDal.DeleteAsync(deletedOrderItem.Data);
            return result != null ? new SuccessResult(Messages.Added) : new ErrorResult(Messages.NotAdded);
        }
        #endregion
    }
